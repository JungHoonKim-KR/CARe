...
export default function ScanPage() {
  ...
  const [allScratches, setAllScratches] = useState([])  // ← 전체 흠집 데이터
  ...
  useEffect(() => {
    ...
    wsRef.current.onmessage = (event) => {
      if (matchStatusRef.current === 'captured') {
  ...
  function getInstructionHtml() {
    if (matchStatus === 'captured') {
      return result.hasDefect
        ? `<strong style="color:#ef4444">흠집 ${result.count}개</strong>가 감지되어 저장되었습니다.<br>다음 구역으로 이동하세요.`
        : `<strong style="color:#10b981">이상 없음</strong> — 깨끗한 상태입니다.<br>다음 구역으로 이동하세요.`
    }
    if (matchStatus === 'matched') {
      return `구역 인식 완료! <strong style="color:#4f46e5">촬영 버튼</strong>을 눌러 흠집을 저장하세요.`
    }
    ...
    <div className={styles.page}>
      ...
      <div className={styles.zoneHeader}>
        ...
        <div className={`${styles.statusBadge} ${styles[matchStatus] || styles.detecting}`}>
          ...
          <span>
            {matchStatus === 'detecting' ? '인식 중...' :
             matchStatus === 'matched'   ? '촬영 대기' :
             '촬영 완료'}
          </span>
        </div>
      </div>
      ...
      <div className={styles.instruction}>
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: getInstructionHtml() }}
        />
      </div>
